...
async def _add_mentor(db, slack_id, name="Coach Ray", is_active=True):
    m = Mentor(name=name, slack_user_id=slack_id, member_code="mnt00001", is_active=is_active)
    db.add(m)
    await db.commit()
    await db.refresh(m)
    return m
...
async def test_gtfo_with_no_open_sessions(client, db, monkeypatch):
    ...
    assert resp.status_code == 200
    assert "No students were signed in" in resp.text
    assert calls == [[]]
...
async def test_gtfo_writes_audit_row(client, db, make_student, monkeypatch):
    ...
    assert "Coach Ray signed out 1 student(s) via /gtfo" in rows[0].summary
